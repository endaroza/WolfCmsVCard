...
/**
 * The VCardReader class encapsulates a single vCard object by maintaining a map of
...
class VCardProperty
{
    ...
    function parse(&$lines)
    {
        while (list(, $line) = each($lines)) {
            ...
            if (count($tmp) == 2) {
                ...
                if (array_key_exists('ENCODING', $this->params) && sizeof($this->params['ENCODING'] > 0)) {
                    if ($this->params['ENCODING'][0] == 'QUOTED-PRINTABLE') {
                        $this->_decodeQuotedPrintable($lines);
                    }
                }
                if (array_key_exists('CHARSET', $this->params) && sizeof($this->params['CHARSET'] > 0)) {
    ...
    {
        static $types = array('DOM', 'INTL', 'POSTAL', 'PARCEL', 'HOME', 'WORK', 'PREF', 'VOICE', 'FAX', 'MSG', 'CELL', 'PAGER', 'BBS', 'MODEM', 'CAR', 'ISDN', 'VIDEO', 'AOL', 'APPLELINK', 'ATTMAIL', 'CIS', 'EWORLD', 'INTERNET', 'IBMMAIL', 'MCIMAIL', 'POWERSHARE', 'PRODIGY', 'TLX', 'X400', 'GIF', 'CGM', 'WMF', 'BMP', 'MET', 'PMB', 'DIB', 'PICT', 'TIFF', 'PDF', 'PS', 'JPEG', 'QTIME', 'MPEG', 'MPEG2', 'AVI', 'WAVE', 'AIFF', 'PCM', 'X509', 'PGP');
